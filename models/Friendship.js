...
async function setUpViews(userId) {
    ...
    latestEventQuery = "CREATE OR REPLACE VIEW latestEvents as (\
        select a.*, b.array_agg from friendEventsWithUser a, latestEventWithUser b \
        where a.combined = b.combined_inner and a.status_id = ANY(b.array_agg)\
        );"
...
async function getRequestsIncoming(userId) {
    ...
    incomingRequestsQuery = "SELECT a.*, b.username FROM latestEvents a, user_timeout b where \
...
async function getRequestsOutgoing(userId) {
    ...
    await setUpViews(userId)
    outgoingRequestsQuery = "SELECT a.*, b.username FROM latestEvents a, user_timeout b where \
